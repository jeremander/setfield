...
class SubsetComplement(FilterSubset[T]):
    ...
    def __repr__(self) -> str:
        return f'{type(self).__qualname__}({self.subset!r})'
...
Signs: TypeAlias = tuple[bool, ...]


def get_atoms(subsets: Iterable[BaseSubset[T]]) -> list[tuple[Signs, Subset[T]]]:
    """Gets the atoms of the boolean algebra generated by subsets of the same (finite) universe.
    Returns a list of (signs, atom) pairs, where the signs are a tuple of booleans matching the length of the input
    sequence, indicating whether each subset is present or absent in the atom."""
    subset_list = list(subsets)
    if not subset_list:
        return []
    if (universe := subset_list[0].universe) is None:
        raise ValueError('universe must be finite')
    if any((subset.universe != universe) for subset in subset_list[1:]):
        raise ValueError('all subsets must have the same universe')
    partition: list[tuple[Signs, set[T]]] = [((), set(universe))]
    for subset in subset_list:
        subset_elts = set(subset)
        new_partition: list[tuple[Signs, set[T]]] = []
        for (signs, block) in partition:
            if (inside := block & subset_elts):
                new_partition.append((signs + (True,), inside))
            if (outside := block - subset_elts):
                new_partition.append((signs + (False,), outside))
        partition = new_partition
    return [(signs, Subset(universe, block)) for (signs, block) in partition]
